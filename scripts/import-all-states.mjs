#!/usr/bin/env node
/**
 * Import All States - Nationwide Pizza Place Import
 *
 * Loops through all 50 US states and imports pizza places from OpenStreetMap.
 * Uses progress tracking for resume capability and rate limiting to respect API limits.
 *
 * Usage:
 *   node scripts/import-all-states.mjs               # Run full import
 *   node scripts/import-all-states.mjs --dry-run     # Preview without inserting
 *   node scripts/import-all-states.mjs --limit=5     # Only process 5 states
 *   node scripts/import-all-states.mjs --resume      # Resume from last checkpoint
 *   node scripts/import-all-states.mjs --report      # Show progress report only
 */
...
function importState(stateName, stateCode, dryRun = false) {
  return new Promise((resolve, reject) => {
    const args = [
      join(__dirname, 'import-osm-pizza.mjs'),
      '--state', stateName,
...
async function main() {
  ...
  console.log('NATIONWIDE PIZZA PLACE IMPORT')
  console.log('='.repeat(60))
  console.log('')

  // Initialize tracker
  const tracker = new StateImportTracker()
  await tracker.load()
  ...
  // Get states to process (skip Michigan - already done)
  let statesToProcess = tracker.getStatesToProcess(US_STATES, true)
