#!/usr/bin/env node
/**
 * Retry Failed Caribbean Queries
 *
 * Retries the 6 queries that failed with 504 errors during the Caribbean import.
 */
...
// Failed queries from Caribbean import (stateCode must be <=10 chars)
const FAILED_QUERIES = [
  { country: 'Dominican Republic', stateCode: 'DO', city: 'Santiago', iso: null, bbox: [19.4, -70.8, 19.6, -70.6], type: 'tacos' },
  { country: 'Saint Lucia', stateCode: 'LC', city: null, iso: 'LC', bbox: null, type: 'pizza' },
  { country: 'British Virgin Islands', stateCode: 'VG', city: null, iso: 'VG', bbox: null, type: 'pizza' },
  { country: 'Turks and Caicos', stateCode: 'TC', city: null, iso: 'TC', bbox: null, type: 'tacos' },
  { country: 'Martinique', stateCode: 'MQ', city: null, iso: 'MQ', bbox: null, type: 'pizza' },
  { country: 'Bonaire', stateCode: 'BQ', city: null, iso: 'BQ', bbox: null, type: 'pizza' },
]
...
function extractPlaces(data, type, stateCode) {
  const places = []
  ...
  for (const element of data.elements || []) {
    ...
    const place = {
      ...
      lat: lat,
      lng: lon,
      state: tags['addr:state'] || stateCode,
...
async function main() {
  console.log('=== Retrying Failed Caribbean Queries ===')
  ...
  for (let i = 0; i < FAILED_QUERIES.length; i++) {
    ...
    const location = q.city ? `${q.city}, ${q.country}` : q.country

    console.log(`[${i + 1}/${FAILED_QUERIES.length}] ${location} - ${q.type}...`)
    ...
    try {
      ...
      const places = extractPlaces(data, q.type, q.stateCode)

      const table = q.type === 'pizza' ? 'pizza_places' : 'taco_places'
      const inserted = await upsertPlaces(places, table)
