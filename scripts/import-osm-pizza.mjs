 ...
 *   node scripts/import-osm-pizza.mjs --state "Lombardy" --state-code "LOM" --admin-level 3  # European regions
 */
 ...
 * @param {string} stateName - Full state/region name for OSM query
 * @param {string} adminLevel - OSM admin_level (default '4' for US states, '3' for European regions)
 */
 ...
 * @param {string} adminLevel - OSM admin_level (default '4' for US states, '3' for European regions)
 */
async function fetchOsmPizzaPlaces(stateName, stateCode, adminLevel = '4') {
...
async function main() {
  ...
  console.log(`=== Importing pizza places from ${stateName} (${stateCode}) [admin_level=${adminLevel}] ===\n`)
  ...
  try {
    ...
    // Fetch from OSM
    const osmPlaces = await fetchOsmPizzaPlaces(stateName, stateCode, adminLevel)

    // Load local cache (zero egress!)
    console.log('Loading local OSM ID cache...')
    const cache = await loadOsmIdCache()
    console.log(`Cache has ${cache.pizza.size} existing pizza OSM IDs`)

    // Deduplicate using local cache
    console.log('\nDeduplicating against local cache...')
    const { toInsert, skipped } = deduplicateWithCache(osmPlaces, cache.pizza)
    ...
    if (!dryRun && !sqlMode && toInsert.length > 0) {
      ...
      await appendToOsmIdCache('pizza', toInsert)
    } else if (dryRun) {
