...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (arg === '--audit-manifest') args.auditManifest = argv[++i];
...
Options:
  ...
  --audit-manifest <f>   Audit an ATP spider manifest against the current run
...
function loadManifest(manifestPath) {
  return JSON.parse(readFileSync(resolve(process.cwd(), manifestPath), 'utf8'));
}

function auditManifest(manifest, results) {
  const bySpider = new Map(results.map(result => [normalizeResult(result).spider, normalizeResult(result)]));
  const rows = (manifest.spiders || []).map(entry => {
    const stats = bySpider.get(entry.spider);
    const features = stats ? stats.features : null;
    const errors = stats ? stats.errors : null;
    const run_status = !stats
      ? 'missing'
      : features > 0
        ? 'present'
        : 'empty';
    const import_status = entry.import_enabled
      ? run_status === 'present'
        ? errors > 0
          ? 'enabled_with_errors'
          : 'enabled_ok'
    ...
    return {
      ...
      import_enabled: Boolean(entry.import_enabled),
      run_status,
      features,
      errors,
      import_status,
      note: entry.note || entry.brand || '',
    };
  });

  const summary = rows.reduce((acc, row) => {
    acc[row.import_status] = (acc[row.import_status] || 0) + 1;
    return acc;
  }, {});
...
function printManifestAudit(audit) {
  if (!audit) return;
  console.log('## Manifest Audit');
  if (audit.manifest.entity) console.log(`entity: ${audit.manifest.entity}`);
  if (audit.manifest.source) console.log(`source: ${audit.manifest.source}`);
  if (audit.manifest.updated_at) console.log(`manifest_updated_at: ${audit.manifest.updated_at}`);
  console.log('');
  console.log('| status | rows |');
  console.log('| --- | ---: |');
  for (const [status, count] of Object.entries(audit.summary).sort(([a], [b]) => a.localeCompare(b))) {
    console.log(`| ${status} | ${count} |`);
  }
  console.log('');
  console.log('| spider | group | manifest_status | import_enabled | run_status | features | errors | import_status | note |');
  console.log('| --- | --- | --- | --- | --- | ---: | ---: | --- | --- |');
  for (const row of audit.rows) {
    console.log(`| ${row.spider} | ${row.group} | ${row.manifest_status} | ${row.import_enabled ? 'yes' : 'no'} | ${row.run_status} | ${row.features ?? '-'} | ${row.errors ?? '-'} | ${row.import_status} | ${row.note} |`);
  }
  console.log('');
}

function printMarkdown({ latest, outputBaseUrl, rowsByTerm, args, manifestAudit }) {
  ...
  printManifestAudit(manifestAudit);
}
...
async function main() {
  ...
  const manifestAudit = args.auditManifest ? auditManifest(loadManifest(args.auditManifest), results) : null;
  ...
  else printMarkdown({ latest, outputBaseUrl, rowsByTerm: matches.rowsByTerm, args, manifestAudit });
}
