...
```bash
ssh example-host 'cd /srv/apizzamichigan && node scripts/ops/supabase-sync-readiness-report.mjs --entity taco --batch 25 --json'
ssh example-host 'cd /srv/apizzamichigan && node scripts/ops/supabase-sync-status-report.mjs --entity taco --json'
```
